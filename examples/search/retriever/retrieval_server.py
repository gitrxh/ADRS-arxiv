...
class DenseRetriever(BaseRetriever):
    def __init__(self, config):
        ...
        # Progress logging for slow index/corpus loading.
        
        _t = time.time()
        print(f"[retriever] reading faiss index from {self.index_path} ...", flush=True)
        self.index = faiss.read_index(self.index_path)
        print(f"[retriever] faiss index loaded in {time.time() - _t:.1f}s (ntotal={self.index.ntotal})", flush=True)
        if config.faiss_gpu and hasattr(faiss, "GpuMultipleClonerOptions"):
            _t = time.time()
            print("[retriever] moving index to GPU(s) ...", flush=True)
            co = faiss.GpuMultipleClonerOptions()
            co.useFloat16 = True
            co.shard = True
            self.index = faiss.index_cpu_to_all_gpus(self.index, co=co)
            print(f"[retriever] index moved to GPU in {time.time() - _t:.1f}s", flush=True)
        elif config.faiss_gpu:
            # Gracefully fall back to CPU if faiss-gpu is unavailable.
            
            print("[retriever] WARNING: faiss has no GPU support (faiss-cpu installed?); "
                  "falling back to CPU index (retrieval will be slower).", flush=True)

        _t = time.time()
        print(f"[retriever] loading corpus from {self.corpus_path} ...", flush=True)
        self.corpus = load_corpus(self.corpus_path)
        print(f"[retriever] corpus loaded in {time.time() - _t:.1f}s (size={len(self.corpus)})", flush=True)

        _t = time.time()
        print("[retriever] loading encoder model ...", flush=True)
        ...
        print(f"[retriever] encoder loaded in {time.time() - _t:.1f}s — retriever READY", flush=True)
        self.topk = config.retrieval_topk
