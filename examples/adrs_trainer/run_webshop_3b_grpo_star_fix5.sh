...
# STAR fix5: fix4 + entropy protection (wn_clip + 3-phase schedule)
...
# additive (default) = z-score additive within_norm; multiplicative = RLSD/Distilled-RL style ratio gate.
# Under multiplicative mode, eta acts as the RLSD mixing coefficient and is not comparable to additive eta (typically much larger).
...
experiment_name="grpo_star_fix5${GATE_TAG}${RTG_TAG}${CLIP_TAG}${SIGN_TAG}${ENT_TAG}${ADAPT_TAG}${KLBOOST_TAG}${MODE_TAG}${NEGRESET_TAG}_perseq_klgrpo_webshop_qwen2.5_3b_eta${adrs_eta}_${timestamp}"
...
python3 -m verl.trainer.main_adrs \
    ...
    trainer.project_name='opto_agent_webshop' \
    ...
    trainer.test_freq=5 \
    trainer.total_epochs=${TOTAL_STEPS:-150} \
