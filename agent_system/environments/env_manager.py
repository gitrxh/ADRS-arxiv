...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        eval_dataset = getattr(config.env.alfworld, 'eval_dataset', 'eval_in_distribution')
        eval_split = getattr(config.env, 'eval_split', None)
        if eval_split == 'unseen':
            eval_dataset = 'eval_out_of_distribution'
        elif eval_split == 'seen':
            eval_dataset = 'eval_in_distribution'

        data_fraction = float(getattr(config.env, 'data_fraction', 1.0))
        print(f"[make_envs] alfworld: eval_dataset={eval_dataset}, data_fraction={data_fraction}, eval_split={eval_split}")
