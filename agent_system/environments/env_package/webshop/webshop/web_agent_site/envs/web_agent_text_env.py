...
import string
import time
# torch 仅在 get_image 模式下用到(加载图像特征)。文本模式(本项目唯一用法)不需要它,
# 而每个 webshop worker 都是独立进程,顶层 import torch 会让 ~256 个 worker 各驮一份
# torch(~0.3-1G RSS/进程)纯属浪费 CPU 内存。改为在真正需要处 lazy import。
...
class WebAgentTextEnv(gym.Env):
    ...
    def __init__(
        ...
        if self.kwargs.get('get_image', 0):
            import torch  # lazy: 仅 get_image 模式需要
