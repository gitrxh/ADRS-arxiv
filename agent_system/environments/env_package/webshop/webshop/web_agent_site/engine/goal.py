...
# 集群上 en_core_web_sm 按包名查找反复失败(镜像 pip 装 user site / OSS FUSE 拷贝不稳,
# 导致 spacy.load("en_core_web_sm") 报 E050)。改为优先读环境变量 WEBSHOP_SPACY_MODEL
# 指定的*模型数据目录绝对路径*(train.sh 从 OSS 拷到本地盘并 export),彻底绕开包发现机制;
# 未设时回退按包名加载(本地开发/已正确安装的环境)。
nlp = spacy.load(os.environ.get("WEBSHOP_SPACY_MODEL") or "en_core_web_sm")
