...
def load_products(filepath, attrpath, num_products=None, human_goals=True):
    ...
    # human_attributes (items_human_ins.json) is only needed when human_goals=True.
    # The 1000-item subset ships without that file, so load it conditionally instead
    # of unconditionally (the original code opened it twice, ignoring human_goals).
    human_attributes = {}
    if human_goals:
