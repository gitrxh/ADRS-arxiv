...
class WebshopWorker:
    ...
    def reset(self, idx):
        ...
        info['available_actions'] = self.env.get_available_actions()
        info['won'] = False
        # 防内存泄漏:SimServer.user_sessions 每个新 session 只增不删,长训练会
        # 线性吃满 CPU 内存直至 OOM(webshop obs 大,实测 ~1.5GB/step,70 步即崩)。
        # 每个 worker 同时只跑一个 session,历史 session 的 goal/verbose_info 无用,
        # reset 后只保留当前 session。
        try:
            base = getattr(self.env, 'unwrapped', self.env)
            server = getattr(base, 'server', None)
            cur = getattr(base, 'session', None)
            if server is not None and hasattr(server, 'user_sessions') and len(server.user_sessions) > 1:
                for sid in [s for s in server.user_sessions if s != cur]:
                    del server.user_sessions[sid]
        except Exception:
            pass
        # 每步对 HTML 做 BeautifulSoup 解析会产生大量带循环引用的对象,只靠 refcount
        # 回收不掉,长训练下累积撑大 RSS。每个 episode reset 时主动 gc 一次(频率低,开销可忽略)。
        gc.collect()
        return obs, info
