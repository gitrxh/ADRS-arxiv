...
class AlfredTWEnv(object):
    ...
    def collect_game_files(self, verbose=False):
        ...
        if self.train_eval == "train":
            ...
            data_fraction = self.config['dataset'].get('_data_fraction', 1.0)
            if 0 < data_fraction < 1.0:
                num_train_games = max(1, int(len(self.game_files) * data_fraction))
            self.game_files = self.game_files[:num_train_games]
            self.num_games = len(self.game_files)
            print("Training with %d games (fraction=%.2f)" % (len(self.game_files), data_fraction))
        else:
