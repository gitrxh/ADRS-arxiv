...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: Union[str, List[str]] = "console", config=None):
        ...
        # Always write metrics JSONL to METRICS_CSV_DIR (if set)
        import os
        metrics_dir = os.environ.get("METRICS_LOG_DIR", "")
        if metrics_dir:
            os.makedirs(metrics_dir, exist_ok=True)
            metrics_path = os.path.join(metrics_dir, f"{experiment_name}.jsonl")
            self._metrics_file = open(metrics_path, "a")
            print(f"[Tracking] Metrics JSONL: {metrics_path}")
        else:
    ...
    def __del__(self):
        if hasattr(self, '_metrics_file') and self._metrics_file:
