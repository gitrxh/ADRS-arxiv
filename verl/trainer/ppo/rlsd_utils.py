...
class SkillProvider:
    ...
    def get_privileged_info_from_data_source(self, data_source: str, prompt_text: str) -> str:
        ...
        Matching rules (search-style QA data sources):
          ...
          - data_source in ('2wikimultihopqa', 'musique', 'bamboogle') -> multi_hop_reasoning
          - otherwise -> fall back to prompt keyword inference (see below).

        The fallback is essential for non-search envs (alfworld/webshop), whose
        ``data_source`` is the modality tag ("text"/"visual") produced by the
        verl-agent dummy dataset, not a QA dataset name. Such tags never match a
        search rule, so without the fallback the teacher would *only ever* see
        ``general_skills`` and never the task-specific skill. We therefore defer
        to ``task_keywords``-based inference whenever the data_source cannot be
        mapped to a skill that actually exists in this environment's skill set.
        """
        if self.skill_all:
            return self._all_skills_text
        ...
        # Fall back to prompt keyword inference when the data_source doesn't map
        # to a skill present in this environment's skill set -- e.g. modality
        # tags like "text" used by alfworld/webshop, or a misfired "compare"
        # guess in an env that has no compare skill. This is what makes
        # task-specific skills actually reach the teacher for non-search envs.
        mapped_name = self.task_to_skill.get(task_type) if task_type else None
        if mapped_name is None or mapped_name not in self.skill_contents:
...
def compute_rlsd_token_advantage(
    ...
    Args:
        ...
        neg_reset: if True, do not apply teacher modulation on negative-advantage
            samples (A_seq <= 0). These samples keep their original seq-level advantage
            unchanged. Inspired by Distilled RL's "negative sample reset".
    ...
    with torch.no_grad():
        ...
        if neg_reset:
            neg_mask = (A_seq <= 0).unsqueeze(-1)  # (bs, 1)
            token_advantages = torch.where(neg_mask, seq_advantages, token_advantages)
