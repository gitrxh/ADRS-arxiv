...
def compute_sdar_loss(
    ...
    gate_beta: float = 5.0,
    external_gate: torch.Tensor = None,
    ...
    Args:
        ...
        external_gate: (bs, response_length) optional. If provided, used directly as the
            per-token gate g_t (e.g. TVA competence modulation σ(τ·TVA)) instead of the
            δ-based gate σ(β·δ). This turns SDAR into TVA-gated distillation. Detached internally.
    ...
    # Use external_gate (e.g. TVA competence) if provided; otherwise use SDAR delta-gate.
    if external_gate is not None:
        gate = external_gate.detach()
    else:
